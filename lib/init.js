...
const go = function *(input) {
  ...
  if (path) {
    ...
    const {projectName, projectVersion, projectDescription, projectAuthor} = answers;
    const metaData = {
      projectName,
      projectVersion,
      projectDescription,
    ...
    downloadTemplate(path, projectName, metaData, isInstall);
  }  else {
...
function downloadTemplate(path, projectName, metaData, isInstall) {
  ...
  download(path, targetPath, (err) => {
    ...
    startBuildProject(spanner, targetPath, metaData, isInstall)
  })
}

async function startBuildProject(spanner, targetPath, metaData, isInstall){
  await generator(metaData, targetPath, targetPath);
  if (isInstall) {
